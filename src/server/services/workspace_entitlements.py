...
class WorkspaceEntitlementsMixin:
    ...
    async def _maybe_reclaim_lazy_tier(
        ...
        if sandbox_id:
            try:
                ...
                # The desired postcondition already holds: there is no
                # outsized runtime left to orphan when replacement begins.
                pass
        ...
        *,
        ...
        async with self._observed_lock(computer_id, "computer.spec"):
            async with AsyncExitStack() as stack:
                ...
                if not acquired:
                    raise RuntimeError("Computer is busy; retry the spec change")
                return await self._set_computer_spec_locked(
                    computer_id, tier, user_id=user_id, workspace_id=workspace_id
                )
    ...
    async def _set_computer_spec_locked(
        ...
        *,
        ...
        # Read and compensate under the same cross-worker lock. A failed
        # request must never restore a tier read before another request won.
        computer = await get_computer(computer_id)
        if not computer:
            raise ValueError(f"Computer {computer_id} not found")
        ...
        *,
        ...
        if not locked_sandbox_id:
            ...
            await self._replace_stopped_sandbox(
                replace(binding, resource_tier=locked.get("resource_tier") or "standard"),
                locked_sandbox_id, disk_guard=disk_guard,
                origin_workspace_id=locked.get("origin_workspace_id"),
                user_id=user_id or locked.get("user_id"),
            )
        else:
    ...
    async def _replace_stopped_sandbox(
        self, binding: ComputerBinding, sandbox_id: str, *,
        ...
        if not await try_claim_computer_for_start(
            computer_id, from_status="stopped", expected_provider_ref=sandbox_id,
            require_provider_ref=True,
        ):
            raise RuntimeError("Computer changed while preparing the spec change; retry")
        session = Session(
        ...
        try:
            try:
                ...
                await self._backup_machine_files_to_db(
                    computer_id, strict=True, expected_sandbox_id=sandbox_id, session=session,
                )
