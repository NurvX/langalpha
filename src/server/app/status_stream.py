...
SubscriptionKeyFn = Callable[[], object]
...
async def status_event_stream(
    *,
    ...
    frame: FrameFn,
    subscription_key: SubscriptionKeyFn | None = None,
) -> AsyncIterator[str]:
    ...
    while time.monotonic() - started < EVENTS_MAX_DURATION_S:
        ...
        subscribed_key = subscription_key() if subscription_key else None
        async with subscribe() as wait_for_notify:
            if wait_for_notify is not None:
                ...
                if close:
                    return
                if subscription_key and subscription_key() != subscribed_key:
                    continue
