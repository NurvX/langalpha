...
class DirectMcpPolicyMiddleware(AgentMiddleware):
    ...
    Main-agent only. The check reads the connection when the call runs rather
    than when it is proposed, so an order the user was already asked about can
    still be refused here; ``direct_tool_middleware`` says why that is the
    order. Any tool without the binder's stamp passes through untouched.
    """
