...
This gate wraps the consent gate rather than following it, and consent is read
at execution time, so an order the connection stopped covering mid-turn is
proposed and put to the user before the inner gate refuses it. That is the
order ``direct_tool_middleware`` argues for, since the refusal then settles the
attempt with a receipt instead of leaving it approved with a call nothing ran.
A row that a disconnect or a change of consent already refused never reaches
the interrupt at all, because it is no longer awaiting a decision. Plan
approval keeps the stock human-in-the-loop path untouched.
"""
