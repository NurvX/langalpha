...
class TestModelPricingResolution:
    ...
    def test_tiered_openai_resolves_and_codex_oauth_inherits(self):
        """These use short/long-context tiered pricing on `openai`; the
        codex-oauth twins carry no pricing list and inherit the parent."""
        for model_id in _TIERED_OPENAI_IDS:
    ...
    def test_tiered_openai_threshold_billing_structure(self):
        """OpenAI bills long context threshold-style ("2x input and 1.5x output
        for the full request" past 272K), so the entries must opt into
        threshold input mode and carry per-tier cache-write rates (values
