...
def test_error_workspaces_backfill_as_recoverable_stopped_computers(migration):
    sql = _upgrade_sql(migration)
    assert "CASE WHEN status = 'error' THEN 'stopped' ELSE status END" in sql
    assert (
        "CASE WHEN status = 'error' THEN COALESCE(stopped_at, NOW()) "
        "ELSE stopped_at END" in sql
    )
...
def test_a_shared_sandbox_id_keeps_the_most_recently_active_row(migration):
    ...
    sql = _upgrade_sql(migration)
    assert "SELECT DISTINCT ON (COALESCE(config->>'sandbox_provider', 'daytona'), sandbox_id)" in sql
    assert (
        "ORDER BY COALESCE(config->>'sandbox_provider', 'daytona'), sandbox_id, COALESCE(last_activity_at, updated_at, "
...
def test_the_backfill_re_elects_nobody_on_a_second_run(monkeypatch, migration):
    ...
    assert (
        ...
        "WHERE c.kind = COALESCE(w.config->>'sandbox_provider', 'daytona') AND c.provider_ref = w.sandbox_id "
...
def test_live_table_indexes_do_not_block_rolling_deploy_writers(migration):
    module, op = migration
    module.upgrade()
    sql = _flat(op)
    assert "CREATE INDEX CONCURRENTLY idx_workspaces_computer " in sql
    assert "CREATE UNIQUE INDEX CONCURRENTLY idx_workspaces_computer_dir " in sql
    assert "CREATE INDEX CONCURRENTLY idx_sandbox_egress_grants_computer " in sql
    assert "DROP INDEX CONCURRENTLY IF EXISTS idx_workspaces_computer " in sql
    assert "SET lock_timeout = 0" in sql
    op.get_context.return_value.autocommit_block.assert_called_once_with()


@pytest.mark.parametrize(("yaml_text", "override", "api_key", "expected"), [
    ("sandbox: {provider: docker}", "", "present", "docker"),
    ("sandbox: {provider: docker}", "daytona", "", "daytona"),
    ("sandbox: {}", "", "", "docker"),
    ("sandbox: {}", "", "present", "daytona"),
    ("daytona: {}", "", "", "daytona"),
    ("sandbox: {provider: '${TEST_PROVIDER}'}", "", "", "docker"),
])
def test_backfill_provider_matches_config(monkeypatch, migration, tmp_path,
                                         yaml_text, override, api_key, expected):
    config = tmp_path / "agent_config.yaml"
    config.write_text(yaml_text)
    monkeypatch.setenv("PTC_CONFIG_FILE", str(config))
    monkeypatch.setenv("SANDBOX_PROVIDER", override)
    monkeypatch.setenv("DAYTONA_API_KEY", api_key)
    monkeypatch.setenv("TEST_PROVIDER", "docker")
    assert migration[0]._backfill_kind() == expected


def test_backfill_rejects_unknown_provider(monkeypatch, migration):
    monkeypatch.setenv("SANDBOX_PROVIDER", "unsupported")
    with pytest.raises(ValueError, match="Unknown sandbox provider"):
        migration[0]._backfill_kind()


def test_backfill_reads_local_yaml_without_overrides(monkeypatch, migration, tmp_path):
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("PTC_CONFIG_FILE", raising=False)
    monkeypatch.delenv("SANDBOX_PROVIDER", raising=False)
    (tmp_path / "agent_config.yaml").write_text("sandbox: {provider: docker}")
    assert migration[0]._backfill_kind() == "docker"


def test_backfill_root_uses_yaml_even_with_provider_override(monkeypatch, migration, tmp_path):
    config = tmp_path / "agent_config.yaml"
    config.write_text("filesystem: {working_directory: '${TEST_SANDBOX_ROOT}'}")
    monkeypatch.setenv("PTC_CONFIG_FILE", str(config))
    monkeypatch.setenv("SANDBOX_PROVIDER", "docker")
    monkeypatch.setenv("TEST_SANDBOX_ROOT", "/srv/old-root")
    assert migration[0]._backfill_root() == "/srv/old-root"
    sql = _upgrade_sql(migration)
