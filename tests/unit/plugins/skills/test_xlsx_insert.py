    ...
    [
        (
            ...
            dict(
                ...
                cross="X!$B$3+SUM('X'!B3:D9)+SUM(Start:End!B3:D8)+SUM(X!B:D)+SUM(X!3:9)",
                single="D5",
                m12="M13",
            ),
        ),
        (
            ...
            dict(
                ...
                cross="X!$B$3+SUM('X'!B3:D7)+SUM(Start:End!B3:D8)+SUM(X!B:D)+SUM(X!3:7)",
                single="#REF!",
                m12="M11",
            ),
        ),
        (
            ...
            dict(
                ...
                cross="X!$B$3+SUM('X'!B3:E8)+SUM(Start:End!B3:D8)+SUM(X!B:E)+SUM(X!3:8)",
                single="E4",
                m12="N12",
            ),
        ),
        (
            ...
            dict(
                ...
                to_rc=(9, 10),
                vml=None,
                cross="X!$B$3+SUM('X'!B3:C8)+SUM(Start:End!B3:D8)+SUM(X!B:C)+SUM(X!3:8)",
                single="#REF!",
                m12="L12",
            ),
        ),
...
def test_comprehensive_four_operations(tmp_path, op, at, expected):
    ...
    # Column D is the table's Cost column, so deleting it takes Sales[Cost] with it.
    sales = "#REF!" if op == "delete-columns" else "Sales[Cost]"
    assert (
        primary
        == f'SUM({abs_band})+SUM({expected["rowband"]})+SUM({expected["colband"]})+LOG10(B3)+ATAN2(B3,C3)+IF(B3>0,"B3",0)+{sales}+Stable'
    )
    assert formula(root, master_ref[:-1] + "3") == expected["single"]
    assert (
        formula(root, expected["m12"])
        == "'Other Sheet'!B4+Start!B4+SUM(Start:End!B3:D8)"
    )
    other = xml(after, "xl/worksheets/sheet4.xml")
    assert formula(other, "A1") == expected["cross"]
    before_a2 = formula(xml(before, "xl/worksheets/sheet4.xml"), "A2")
    assert formula(other, "A2") == before_a2.replace("Sales[Cost]", sales)
    names = {
        n.get("name"): n.text
...
@pytest.mark.parametrize(
    ...
    [
        (
            "row",
            3,
            "SUM('X'!$B4, X!B$5, Start:End!B3, 'Start:End'!B3, Other!B3, [1]X!B3, '[book.xlsx]X'!B3, Sales[Cost], Stable, LOG10(B4), ATAN2(B4,C4), \"B3 #REF!\", #N/A)  +  B4",
        ),
        (
            "column",
            2,
            "SUM('X'!$C3, X!C$4, Start:End!B3, 'Start:End'!B3, Other!B3, [1]X!B3, '[book.xlsx]X'!B3, Sales[Cost], Stable, LOG10(C3), ATAN2(C3,D3), \"B3 #REF!\", #N/A)  +  C3",
        ),
    ],
)
def test_tokenizer_protects_non_references(axis, at, expected):
    ...
    assert not shift.three_d("End:Other") and shift.three_d("End:Start")
...
def test_quoted_sheet_escaped_apostrophe_and_3d():
    ...
    assert (
        ...
        == "'O''Brien'!A4+'First Sheet:Last Sheet'!A3+'First Sheet':'Last Sheet'!A3"
    )
    apostrophe = insert.Shift("Bob'", ["Bob'"], "row", 2, 1, False)
    assert apostrophe.formula("'Bob'''!A2", "Other") == "'Bob'''!A3"


def test_three_d_spans_stay_as_written_and_are_warned(tmp_path):
    def workbook(name):
        path = tmp_path / name
        wb = Workbook()
        wb.active.title = "Jan"
        for n, month in enumerate(["Jan", "Feb", "Mar", "Apr"]):
            sheet = wb["Jan"] if month == "Jan" else wb.create_sheet(month)
            for row in range(1, 4):
                sheet.cell(row, 1, (n + 1) * 10 ** (row - 1))
        summary = wb.create_sheet("Summary")
        summary["B1"] = "=SUM(Jan:Mar!A1)"
        summary["B2"] = "=SUM(Jan:Mar!A1:A3)"
        summary["B3"] = "=Feb!A1"
        summary["B4"] = "=SUM(Feb:Mar!A1)"
        summary["B5"] = "=SUM(Mar:Apr!A1)"
        summary["B6"] = "=SUM(Feb:Feb!A1)"
        summary["B7"] = "=SUM('Jan:Mar'!A1)"
        wb.save(path)
        return path

    path = workbook("span.xlsx")
    edit = ["rows", str(path), "--sheet", "Feb", "--at", "1"]
    dry = insert.execute(edit + ["--dry-run"])
    report = insert.execute(edit)
    assert report["status"] == "success" and report["self_check"] == {"ok": True}
    assert dry["dry_run"] and dry["warnings"] == report["warnings"]
    # Excel repoints the plain Feb reference and leaves every span as written, so each
    # span's Feb component now reads the blank row the insertion put at A1.
    summary = xml(payload(path), "xl/worksheets/sheet5.xml")
    assert [formula(summary, f"B{row}") for row in range(1, 8)] == [
        "SUM(Jan:Mar!A1)",
        "SUM(Jan:Mar!A1:A3)",
        "Feb!A2",
        "SUM(Feb:Mar!A1)",
    ...
    assert report["formulas_rewritten"] == 1
    spans = [w for w in report["warnings"] if w["type"] == "three_d_reference"]
    # Mar:Apr is the one span the edited sheet sits outside of.
    assert [(w["sheet"], w["cell"], w["span"]) for w in spans] == [
        ("Summary", "B1", "Jan:Mar"),
        ("Summary", "B2", "Jan:Mar"),
        ("Summary", "B4", "Feb:Mar"),
        ("Summary", "B6", "Feb:Feb"),
    ...
    assert spans[0]["reference"] == "Jan:Mar!A1"
    assert spans[0]["formula"] == "SUM(Jan:Mar!A1)"

    # An insertion under the cells a span names changes nothing it reads, except for
    # the range that brackets the new row and loses the value that slid out of it.
    path = workbook("below.xlsx")
    report = insert.execute(["rows", str(path), "--sheet", "Feb", "--at", "3"])
    assert report["status"] == "success" and report["self_check"] == {"ok": True}
    assert [
        (w["cell"], w["span"])
        for w in report["warnings"]
        if w["type"] == "three_d_reference"
    ] == [("B2", "Jan:Mar")]
...
def test_out_creates_missing_parent_directory(tmp_path):
    path = basic(tmp_path / "source.xlsx")
    source = path.read_bytes()
    output = tmp_path / "new-task" / "nested" / "model.xlsx"
    report = run(path, "rows", "3", "--out", output)
    assert report["status"] == "success", report
    assert output.exists() and path.read_bytes() == source
    assert load_workbook(output)["X"].max_row == 13
    assert not list(output.parent.glob(".insert-*"))
...
def test_deleted_table_column_structured_references(tmp_path):
    def workbook(name):
        path = basic(tmp_path / name)
        wb = load_workbook(path)
        ws = wb["X"]
        for col, header in zip(range(2, 6), ["Region", "Revenue", "Cost", "Margin"]):
            ws.cell(2, col, header)
        ws.add_table(Table(displayName="Sales", ref="B2:E8"))
        ws["G2"] = "=SUM(Sales[Revenue])"
        ws["G3"] = "=COUNTA(Sales[[#Headers],[Revenue]])"
        ws["G4"] = "=SUM(Sales[[Revenue]:[Cost]])"
        ws["G5"] = "=SUM(Sales[[#Data],[Region]:[Cost]])"
        ws["G6"] = "=Sales[@Revenue]-Sales[@Cost]"
        ws["G7"] = "=SUM(Sales[Cost])+SUM(Sales[#All])"
        wb["Other"]["A1"] = "=SUM(Sales[Revenue])"
        wb.save(path)
        return path

    path = workbook("revenue.xlsx")
    dry = run(path, "delete-columns", "C", "--dry-run")
    report = run(path, "delete-columns", "C")
    assert report["status"] == "success" and report["self_check"] == {"ok": True}
    assert dry["dry_run"] and dry["warnings"] == report["warnings"]
    after = payload(path)
    assert [
        formula(xml(after, "xl/worksheets/sheet1.xml"), f"F{row}")
        for row in range(2, 8)
    ] == [
        "SUM(#REF!)",
        "COUNTA(#REF!)",
        "SUM(Sales[Cost])",
        "SUM(Sales[[#Data],[Region]:[Cost]])",
        "#REF!-Sales[@Cost]",
        "SUM(Sales[Cost])+SUM(Sales[#All])",
    ]
    assert formula(xml(after, "xl/worksheets/sheet2.xml"), "A1") == "SUM(#REF!)"
    table = xml(after, "xl/tables/table1.xml")
    assert [
        c.get("name") for c in table.findall("s:tableColumns/s:tableColumn", NS)
    ] == [
        "Region",
        "Cost",
        "Margin",
    ...
    assert [
        (w["sheet"], w["cell"]) for w in report["warnings"] if w["type"] == "ref_error"
    ] == [("X", "F2"), ("X", "F3"), ("X", "F6"), ("Other", "A1")]

    path = workbook("cost.xlsx")
    report = run(path, "delete-columns", "D")
    assert report["status"] == "success" and report["self_check"] == {"ok": True}
    after = payload(path)
    assert [
        ...
        "SUM(Sales[Revenue])",
        "COUNTA(Sales[[#Headers],[Revenue]])",
        "SUM(Sales[Revenue])",
        "SUM(Sales[[#Data],[Region]:[Revenue]])",
        "Sales[@Revenue]-#REF!",
        "SUM(#REF!)+SUM(Sales[#All])",
    ]
    assert (
        formula(xml(after, "xl/worksheets/sheet2.xml"), "A1") == "SUM(Sales[Revenue])"
    )


def test_dropped_table_structured_references(tmp_path):
    path = basic(tmp_path / "gone_table.xlsx")
    wb = load_workbook(path)
    ws = wb["X"]
    for col, header in zip(range(2, 5), ["Region", "Revenue", "Cost"]):
        ws.cell(2, col, header)
    for col, header in zip(range(6, 9), ["Alpha", "Beta", "Gamma"]):
        ws.cell(2, col, header)
    ws.add_table(Table(displayName="Sales", ref="B2:D8"))
    ws.add_table(Table(displayName="Kept", ref="F2:H8"))
    ws["J2"] = "=SUM(Sales)"
    ws["J3"] = "=ROWS(Sales[#All])"
    ws["J4"] = "=COUNTA(Sales[#Headers])+SUM(sales[#Data])"
    ws["J5"] = "=SUM(Sales[Revenue])"
    ws["J6"] = "=SUM(Kept)+ROWS(Kept[#All])+SUM(Kept[Alpha])"
    ws["J7"] = '=SUM(F3:F8)+IF(F3>0,"Sales",0)'
    wb["Other"]["A1"] = "=SUM(Sales)-ROWS(Sales[#All])"
    wb.save(path)

    dry = run(path, "delete-columns", "B", "--count", "3", "--dry-run")
    report = run(path, "delete-columns", "B", "--count", "3")
    assert report["status"] == "success" and report["self_check"] == {"ok": True}
    assert dry["dry_run"] and dry["warnings"] == report["warnings"]
    after = payload(path)
    assert [
        formula(xml(after, "xl/worksheets/sheet1.xml"), f"G{row}")
        for row in range(2, 8)
    ] == [
        "SUM(#REF!)",
        "ROWS(#REF!)",
        "COUNTA(#REF!)+SUM(#REF!)",
        "SUM(#REF!)",
        "SUM(Kept)+ROWS(Kept[#All])+SUM(Kept[Alpha])",
        'SUM(C3:C8)+IF(C3>0,"Sales",0)',
    ]
    assert (
        formula(xml(after, "xl/worksheets/sheet2.xml"), "A1")
        == "SUM(#REF!)-ROWS(#REF!)"
    )
    assert [
        (w["sheet"], w["cell"], w["occurrence"])
        for w in report["warnings"]
        if w["type"] == "ref_error"
    ] == [
        ("X", "G2", 1),
    ...
    assert [w["table"] for w in report["warnings"] if w["type"] == "table_dropped"] == [
        "Sales"
    ]
    assert "xl/tables/table1.xml" not in after
    assert [
        c.get("name")
        for c in xml(after, "xl/tables/table2.xml").findall(
            "s:tableColumns/s:tableColumn", NS
        )
    ] == ["Alpha", "Beta", "Gamma"]
    surviving = load_workbook(path)["X"].tables
    assert list(surviving) == ["Kept"] and surviving["Kept"].ref == "C2:E8"
...
def test_foreign_namespace_formula_elements_left_alone(tmp_path):
    # A <formula> or <f> outside Excel's namespaces is somebody else's payload: rewriting
    # it corrupts their data, and reading it as a formula fails the run over their prose.
    crm = "{urn:acme:crm}"
    path = basic(tmp_path / "foreign.xlsx")
    wb = load_workbook(path)
    wb["X"].conditional_formatting.add("D3:F8", FormulaRule(formula=["X!$B$3>0"]))
    wb.save(path)
    ...
    def patch(data):
        ...
        root = xml(data, part)
        extlist = etree.SubElement(root, f"{{{S}}}extLst")
        ext = etree.SubElement(extlist, f"{{{S}}}ext", uri="{crm-test}")
        etree.SubElement(ext, crm + "formula").text = "#REF! ticket 4821"
        save_xml(data, part, root)
        types = xml(data, "[Content_Types].xml")
        etree.SubElement(
            types,
        ...
        data["customXml/item1.xml"] = (
            ...
            b'<crm:record xmlns:crm="urn:acme:crm"><crm:formula>X!B3</crm:formula>'
            b"<crm:f>SUM(X!B1:B5</crm:f></crm:record>"
        )

    patch_package(path, patch)
    before = payload(path)
    report = run(path, "rows", "3")
    assert report["status"] == "success" and report["warnings"] == [], report
    after = payload(path)
    assert_preserved(before, after, report)
    assert "customXml/item1.xml" not in report["parts_changed"]
    assert after["customXml/item1.xml"] == before["customXml/item1.xml"]
    root = xml(after, "xl/worksheets/sheet1.xml")
    assert root.findtext(f".//{crm}formula") == "#REF! ticket 4821"
    rule = root.find("s:conditionalFormatting", NS)
    assert rule.get("sqref") == "D4:F9"
    assert rule.findtext("s:cfRule/s:formula", namespaces=NS) == "X!$B$4>0"
