...
@pytest.fixture
def start_supervisor(monkeypatch):
    ...
    def _start(backend, *, root=None, **options):
        ...
        config = os.path.join(root or directory, ".agents", "tools", "mcp_client_config.json")
        os.makedirs(os.path.dirname(config), exist_ok=True)
        with open(config, "w") as fh:
            json.dump({"workspace_id": "ws-default", "servers": {name: {} for name in backend.names()}}, fh)
        monkeypatch.setitem(_DEFAULT_CALL_CONFIG, "path", config)
        supervisor.bind()
...
_DEFAULT_CALL_CONFIG = {}

def _call(request_id, server, **kwargs):
    kwargs.setdefault("config_path", _DEFAULT_CALL_CONFIG.get("path", ""))
...
def test_absent_config_file_refuses_until_repaired(start_supervisor, tmp_path):
    harness = start_supervisor(FakeBackend({"alpha"}))
    ...
    with open(missing, "w") as fh:
        json.dump({"workspace_id": "ws-1", "servers": {"alpha": {}}}, fh)
    healed = harness.connect().exchange(
        _call(2, "alpha", workspace_id="ws-1", config_path=missing)
    )
    assert [f["type"] for f in healed] == [p.TYPE_ACK, p.TYPE_REPLY]
...
@pytest.mark.parametrize("content", [None, "{bad", "{}", '{"servers": []}'])
def test_client_missing_config_does_not_use_root_and_can_recover(tmp_path, monkeypatch, content):
    from ptc_agent.core.sandbox import mcp_client_runtime as client

    root_config = tmp_path / ".agents/tools/mcp_client_config.json"
    root_config.parent.mkdir(parents=True)
    root_config.write_text(json.dumps({"workspace_id": "root", "servers": {"sibling": {}}}))
    project_config = tmp_path / "project/.agents/tools/mcp_client_config.json"
    project_config.parent.mkdir(parents=True)
    if content is not None:
        project_config.write_text(content)
    monkeypatch.setattr(client, "_WORK_DIR", str(tmp_path))
    monkeypatch.setattr(client, "_WORKSPACE_VIEW_CACHE", {})
    monkeypatch.setenv("PTC_WORKSPACE_CONFIG", str(project_config))
    with pytest.raises(RuntimeError, match="Retry the turn"):
        client._require_enabled("sibling")
    project_config.write_text(json.dumps({"workspace_id": "project", "servers": {"own": {}}}))
    client._require_enabled("own")
