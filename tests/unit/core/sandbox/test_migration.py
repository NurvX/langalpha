...
class TestV3ToV4Script:
    ...
    def test_every_move_is_guarded(self):
        ...
        assert '[ ! -e "$dst" ] && [ ! -L "$dst" ]; then mv' in script
    ...
    def test_the_computer_keeps_its_own_directories(self):
        script = build_v3_to_v4_script(WORK_DIR, DIR_NAME)
        from ptc_agent.core.sandbox.migration import _COMPUTER_RESERVED

        assert {".agents", ".system", "_internal", "mcp_servers", ".bashrc", ".profile"} <= set(_COMPUTER_RESERVED)
...
class TestV3ToV4AgainstRealFiles:
    ...
    def test_project_dotfiles_and_dangling_links_follow_workspace(self, tmp_path):
        root = tmp_path / "home"
        root.mkdir()
        self._seed(root)
        for name in (".env", ".gitignore", "..project"):
            (root / name).write_text(name)
        (root / ".project").mkdir()
        (root / ".project/config").write_text("project")
        (root / ".dangling").symlink_to("missing")
        (root / ".config").mkdir()
        (root / ".config/home").write_text("home")
        for _ in range(2):
            self._run(root, DIR_NAME)
        for name in (".env", ".gitignore", "..project"):
            assert (root / DIR_NAME / name).read_text() == name
            assert not (root / name).exists()
        assert (root / DIR_NAME / ".project/config").read_text() == "project"
        assert (root / DIR_NAME / ".dangling").is_symlink()
        assert (root / ".config/home").read_text() == "home"
...
class TestMigrationHonesty:
    ...
    @pytest.mark.asyncio
    async def test_the_chain_does_not_return_a_partially_migrated_layout(self):
        runtime = AsyncMock()
        ...
        with pytest.raises(RuntimeError, match="boom"):
            await run_layout_migrations(runtime, WORK_DIR, 1, dir_name=DIR_NAME)


def test_repository_agents_folder_is_not_a_sibling_marker(tmp_path):
    for name in ("repository", "known-sibling"):
        (tmp_path / name / ".agents").mkdir(parents=True)
        (tmp_path / name / "data.txt").write_text(name)
    script = build_v3_to_v4_script(
        str(tmp_path), "project", workspace_dir_names=("known-sibling",),
    )
    for _ in range(2):
        subprocess.run(["sh", "-c", script], check=True)
        assert (tmp_path / "project/repository/data.txt").read_text() == "repository"
        assert (tmp_path / "known-sibling/data.txt").read_text() == "known-sibling"
