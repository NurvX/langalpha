...
**2. Determine Cost of Equity (CAPM)**
   ...
   - Beta: [the `beta` field from `yf_fundamentals` MCP `compare_valuations`, or the peer-set average when the name has too little history]
   - Equity Risk Premium: [Call get_market_risk_premium; it publishes no observation date, so the as-of is the retrieval date, labelled as such]

   Example:
   Cost of Equity = rf + beta x ERP
   rf comes from get_treasury_rates with its as-of date; ERP comes from get_market_risk_premium with the retrieval date as its as-of.
   ```
...
- `yf_analysis` MCP `get_revenue_estimates` and `get_earnings_estimates` for each peer's forward revenue and EPS, the `0y` and `+1y` records' `avg` with `numberofanalysts`; NTM is the two fiscal years blended by the months remaining in the current one, the blend stated in the table note. The feed carries no observation date, so the vintage is the retrieval time, labelled as such
- Forward EBITDA and forward net income have no consensus feed, so EV/EBITDA (NTM) is `n/a` for a peer unless a filing or a dated broker figure supplies it with its source, and P/E (NTM) is price over NTM EPS from the feed above, never market cap over an inferred net income
...
P/E (NTM) = Share Price / NTM EPS (est.)
```
...
Source: fundamentals MCP server, yf_analysis estimates (retrieved [Date]), company filings, our estimates. NTM columns without a consensus feed read n/a.
```
