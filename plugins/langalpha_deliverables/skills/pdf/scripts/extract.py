...
def parse_pages(spec: str, total: int) -> list[int]:
    ...
    A spec that selects nothing is an error, not an empty read: `--pages ""` and `--pages ,`
    asked for pages, and a report of `ok` over no page at all reads as a document with
    nothing in it. A token that is not a number is the same kind of mistake and gets the
    same JSON error rather than a traceback.
    """
    wanted: list[int] = []
...
def merge_stray_signs(row: list[str]) -> int:
    """The joined value keeps the figure's own index, because a statement prints
    the `$` on the first row of a block and not the rest: moving `307,003` into
    the `$` column would leave it one column left of the `250,000` below it and
    split one column of numbers in two. The sign column empties out instead and
    the caller drops it. Prefixes run right to left so a `$` lands on a `(` that
    has already taken its number rather than on the bare bracket.
    """
    merged = 0
...
USAGE = (
    "extract.py <file.pdf> [--pages 1-3] [--tables] [--raw-tables] [--layout] "
    "[--out FILE] [--password PW] [--max-chars N]"
)
...
def parse_args(argv: list[str], takes_value: dict[str, bool], usage: str) -> tuple[list[str], dict[str, str | bool]]:
    ...
    name, but it is never read off another option: `--out --layout` used to write a file
    called `--layout`, read the pages with pdfplumber because the layout switch had been
    eaten, and report all of that as a success. A repeated option is the same kind of
    mistake, since the last one silently won: `--pages 1-2 --pages 3` reported `ok` over
    page 3 alone, with no sign that pages 1 and 2 were asked for and never read.
    """
    args: list[str] = []
...
def main(argv: list[str]) -> None:
    ...
    raw_limit = str(values.get("--max-chars", 4000))
    # A zero or negative limit would report success with every page's text gone or
    # trimmed from the end, so the limit is checked before the file is opened.
    if not raw_limit.isdigit() or int(raw_limit) < 1:
        fail(f"--max-chars must be a whole number of at least 1, got {raw_limit!r}")
