...
# Every element ECMA-376 gives a w:id in the tracked-change family. They all draw on one
# id space, so a new revision has to clear the highest id any of them already holds - and
# a document can easily be topped by a kind this file never writes.
REVISION_ID_TAGS = frozenset({
    # Content, paragraph-mark, table-row and w:numPr revisions.
    "ins", "del", "moveFrom", "moveTo",
    # Move range markers; each *End repeats its *Start's id, so both sit in the space.
    "moveFromRangeStart", "moveFromRangeEnd", "moveToRangeStart", "moveToRangeEnd",
    # The same pairs, around a block-level w:customXml.
    "customXmlInsRangeStart", "customXmlInsRangeEnd",
    "customXmlDelRangeStart", "customXmlDelRangeEnd",
    "customXmlMoveFromRangeStart", "customXmlMoveFromRangeEnd",
    "customXmlMoveToRangeStart", "customXmlMoveToRangeEnd",
    # Cell revisions, which live in w:tcPr instead of in the run stream.
    "cellIns", "cellDel", "cellMerge",
    # Property-change records; w:tblGridChange is the one whose name does not end in PrChange.
    "pPrChange", "rPrChange", "sectPrChange",
    "tblPrChange", "tblPrExChange", "trPrChange", "tcPrChange", "tblGridChange",
    # The legacy numbering revision, still written by older Word versions.
    "numberingChange",
})

# A document arrives from outside, and an entity declaration in one of its parts is a
# directive to the parser rather than data: resolving it lets the file decide what the
# tree contains. Every part is parsed with this, never the default parser.
...
def is_element(node) -> bool:
    """False for the nodes lxml mixes in among the elements.

    An XML comment, a processing instruction and an unresolved entity reference
    are siblings of elements in lxml's tree and carry a factory function in
    `tag` where an element carries its name. A document arrives from outside, so
    any of the three can sit anywhere in any part.
    """
    return isinstance(node.tag, str)


def local(el) -> str:
    """The element's local name, or "" for a node that is not an element.

    Every caller asks whether the name is one it handles, and no OOXML element
    is named "", so the empty string is the answer that makes each of them step
    over a comment, processing instruction or entity reference. Reading the name
    off one instead raises, which is what a single such node anywhere in a part
    used to do to every script here.
    """
    return etree.QName(el).localname if isinstance(el.tag, str) else ""


def iter_elements(root):
    """`root.iter()` over elements only."""
...
class Package:
    ...
    def __init__(self, path: str | Path):
        ...
        with zipfile.ZipFile(self.path) as zf:
            ...
            # A zip carries a record for `word/` and `word/media/` whenever the
            # document has been unpacked and rezipped, and a directory is not a
            # package part: it has no content type and nothing points at it.
            # Word and python-docx skip those records, so the parts here do too,
            # while `infos` keeps them and save() copies them through.
            self.raw = {i.filename: zf.read(i.filename) for i in self.infos if not i.is_dir()}
    ...
    def names(self) -> list[str]:
        return [i.filename for i in self.infos if not i.is_dir()] + self._added

    def tree(self, name: str) -> etree._Element:
        if name not in self._trees:
            self._trees[name] = etree.fromstring(self.raw[name], XML)
    ...
    def save(self, dst: str | Path) -> Path:
        ...
        # The temporary sibling needs somewhere to live, and --out routinely
        # names a work directory the caller has not created yet.
        ...
        try:
            with zipfile.ZipFile(tmp, "w", zipfile.ZIP_DEFLATED) as out:
                for info in self.infos:
                    data = b"" if info.is_dir() else self._serialize(info.filename)
    ...
    def _serialize(self, name: str) -> bytes:
        if name in self._dirty:
            # Serialized through the tree rather than the element, so a part that
            # arrived with a DOCTYPE keeps it. Entity references are parsed but
            # never resolved, and writing the element alone leaves every `&name;`
            # in the part pointing at a declaration no longer in the file.
            tree = self._trees[name].getroottree()
...
def _clone(el):
    return etree.fromstring(etree.tostring(el), XML)
...
def max_revision_id(pkg: Package) -> int:
    """The highest w:id the stories already hand to a tracked change.

    Scanning any narrower than REVISION_ID_TAGS gives the next revision an id
    something else in the document already owns, and a duplicated id makes the
    two indistinguishable to anything addressing a revision by id.
    """
    biggest = 0
    for name in pkg.stories():
        for el in iter_elements(pkg.tree(name)):
            if local(el) not in REVISION_ID_TAGS:
                continue
            try:
