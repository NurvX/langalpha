...
Checks, in order of importance:
    ...
    final        --final only: tracked revisions, comments and the reviewer
                 roster left behind in a document being delivered as the clean
                 copy
...
nodes become `final_revisions`, `final_comments` and `final_people` at `fail`.
"""
...
from docx_parts import NS, REVISION_ID_TAGS, Package, iter_elements, iter_paragraphs, local, para_text, qn

METRIC_SAFE = {"arial", "calibri", "cambria", "times new roman", "courier new"}
# The w:style kinds Word applies without being named, against the element whose
# presence in a story means the document holds content of that kind.
DEFAULT_STYLE_CONTENT = {"paragraph": "p", "character": "r", "table": "tbl", "numbering": "numPr"}
PLACEHOLDERS = [
...
EXAMPLES = 20
# Every revision ECMA-376 defines, read off the one list that already had to be
# complete. A set written out by hand here missed w:tblGridChange and the
# customXml range markers, and a document whose only revision was one of those
# passed --final as the clean copy.
REVISION_TAGS = REVISION_ID_TAGS
...
FINAL_PEOPLE_MESSAGE = ("reviewer names and directory ids in a document delivered as final; "
                        "run comments.py strip first")
TRACKED_MESSAGE = ("unresolved tracked changes; deliberate when the human is reviewing, "
                   "a defect when the document is final")
# redline.py resolves the stories, so an agent reading this finding and running
# `accept` sees the count go down by everything except the comment-body ones.
COMMENT_BODY_NOTE = "comment body; redline.py does not resolve these, comments.py strip removes them"
...
def _add_fonts(scope, names: set[str]) -> None:
...
def _styles_reached(styles, used: set[str]) -> list[etree._Element]:
    """The used styles, plus every style they inherit from through w:basedOn.

    A style that declares no w:rFonts takes its family from the style it is
    based on, so reading only the styles the stories name misses the font Word
    paginates those paragraphs with. A w:basedOn in a document from outside can
    close on itself, hence the visited set rather than a walk to the root.
    """
    by_id = {st.get(qn("w:styleId")): st for st in styles.findall(qn("w:style")) if st.get(qn("w:styleId"))}
    reached: list[etree._Element] = []
    seen: set[str] = set()
    queue = [sid for sid in used if sid in by_id]
    while queue:
        sid = queue.pop()
        if sid in seen:
            continue
        seen.add(sid)
        reached.append(by_id[sid])
        base = by_id[sid].find(qn("w:basedOn"))
        if base is not None and base.get(qn("w:val")) in by_id:
            queue.append(base.get(qn("w:val")))
    return reached


def _default_styles(styles, content: set[str]) -> set[str]:
    """The styles Word applies to content that names no style of its own.

    A paragraph with no w:pStyle still paginates in the w:default="1" paragraph
    style, and a run, table and numbered list do the same for their own kind, so
    a font set there reaches the page without any story naming the style. Each
    kind is seeded only when the stories hold content of it, which keeps a
    template's default table style out of a document that has no table.
    """
    seeded: set[str] = set()
    for style in styles.findall(qn("w:style")):
        sid = style.get(qn("w:styleId"))
        on = (style.get(qn("w:default")) or "0").lower() in ("1", "true", "on")
        if sid and on and DEFAULT_STYLE_CONTENT.get(style.get(qn("w:type")) or "paragraph") in content:
            seeded.add(sid)
    return seeded


def check_fonts(pkg: Package, rep: Report, stats: dict) -> None:
    """Only fonts the document actually reaches, across every story.

    A stock Word template defines styles nothing uses (HTML Preformatted brings
    Courier along), and flagging those trains the agent to ignore the check. A
    header, footer or note paginates on the page like the body, so both the
    fonts it sets directly and the styles it names count as reached, and so does
    every style those inherit from - plus the default style of each kind of
    content present, which applies to everything that names no style.
    """
    names: set[str] = set()
    ...
    for part in sorted(pkg.stories()):
        ...
        for el in iter_elements(root):
            tag = local(el)
            content.add(tag)
            if tag in ("pStyle", "rStyle", "tblStyle") and el.get(qn("w:val")):
                used_styles.add(el.get(qn("w:val")))
        _add_fonts(root, names)
    if pkg.has("word/styles.xml"):
        styles = pkg.tree("word/styles.xml")
        defaults = styles.find(qn("w:docDefaults"))
        scopes = [defaults] if defaults is not None else []
        scopes += _styles_reached(styles, used_styles | _default_styles(styles, content))
        for scope in scopes:
            _add_fonts(scope, names)
    ...
    for name in sorted(names):
        if name.lower() not in METRIC_SAFE and not name.startswith("+"):
            rep.f("fonts_unsafe", "warn",
                  "font is outside the metric-safe set (Arial, Calibri, Cambria, Times New Roman, Courier New); "
...
def check_fields(pkg: Package, rep: Report, stats: dict) -> None:
    root = pkg.tree("word/document.xml")
    instructions = " ".join(
        [(el.text or "") for el in root.iter(qn("w:instrText"))]
        + [el.get(qn("w:instr")) or "" for el in root.iter(qn("w:fldSimple"))]
    )
    # Word reads field keywords without regard to case, so `toc \o "1-3"` is as
    # much a TOC as the uppercase spelling, and PROTOCOL is neither.
    has_toc_field = re.search(r"(?<![A-Za-z])TOC(?![A-Za-z])", instructions, re.IGNORECASE) is not None
...
def _revisions(root) -> list[str]:
    """The name of every revision element in the part, one entry per node."""
    return [local(el) for el in iter_elements(root) if local(el) in REVISION_TAGS]


def revision_parts(pkg: Package) -> list[str]:
    """Every part a tracked change can sit in: the stories and the comment parts.

    A comment body is block content, so a reviewer can leave a w:ins or w:del
    inside one, and counting the stories alone called such a document clean of
    tracked changes.
    """
    return sorted(pkg.stories()) + sorted(n for n in pkg.names() if COMMENT_PART_RE.match(n))


def check_review(pkg: Package, rep: Report, stats: dict) -> None:
    counted = [(name, len(_revisions(pkg.tree(name)))) for name in revision_parts(pkg)]
    stats["tracked_changes"] = sum(n for _name, n in counted)
    if stats["tracked_changes"]:
        finding = rep.f("review_tracked_changes", "info", TRACKED_MESSAGE)
        for name, n in counted:
            if n:
                note = f" ({COMMENT_BODY_NOTE})" if COMMENT_PART_RE.match(name) else ""
                finding.add(f"{name} {n} revisions{note}")
...
def check_final(pkg: Package, rep: Report) -> None:
    ...
    reader was never meant to see. word/people.xml counts as review traffic
    even once every comment is gone: it names the reviewers and the directory
    ids behind them, and nothing in the visible document puts them there.
    """
    for name in sorted(pkg.stories()):
        root = pkg.tree(name)
        found = _revisions(root)
        if found:
            # Named, because a caller who has just run `redline.py accept` and still
            # sees this needs to know which revision it was that did not resolve.
            kinds = ", ".join(sorted({"w:" + tag for tag in found}))
            rep.f("final_revisions", "fail",
                  "tracked changes in a document delivered as final; run redline.py accept or reject first").add(
                      f"{Path(name).name} {len(found)} nodes: {kinds}")
        anchors = sum(1 for el in iter_elements(root) if local(el) in COMMENT_ANCHORS)
        if anchors:
    ...
    if pkg.has(PEOPLE_PART):
        rep.f("final_people", "fail", FINAL_PEOPLE_MESSAGE).add(
            f"{PEOPLE_PART} {len(pkg.tree(PEOPLE_PART))} people")
...
USAGE = "usage: validate.py <file.docx> [--strict] [--final]"
TAKES_VALUE = {"--strict": False, "--final": False}
...
def parse_args(argv: list[str]) -> tuple[list[str], dict[str, str | bool]]:
    ...
    A misspelt --final used to fall through unread, so `validate.py draft.docx
    --fianl --strict` never ran the final checks and called a document still
    carrying tracked changes and comments a clean pass.
    """
    args: list[str] = []
...
def main(argv: list[str]) -> None:
    ...
    try:
        report = validate(path, final="--final" in flags)
    except Exception as exc:  # noqa: BLE001
