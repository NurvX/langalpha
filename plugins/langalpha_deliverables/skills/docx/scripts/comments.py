...
and `reopen` clears it, both across every reply in the thread; `reply` clears
it too, on the thread it joins, because a reply is a new turn in a conversation
someone had called finished, and reports `reopened` when it did; `strip` removes
every comment, its anchors, and word/people.xml, which names the reviewers, for
a final copy. Paragraph indices are redline.py's numbering (every w:p in
document order, tables included).
"""
...
# word/people.xml holds no comment text, only the reviewers who wrote them:
# w15:person carries each author's display name and the directory id behind it,
# so a final copy that keeps the part still names everyone who marked the
# document up. Word regenerates it when a comment is next added.
STRIP_PARTS = COMMENT_PARTS + (PEOPLE,)
...
USAGE = "usage: comments.py list|add|reply|resolve|reopen|strip <file.docx> [...]; --help for the flags"
TAKES_VALUE = {
    "--paragraph": True, "--find": True, "--text": True, "--to": True, "--id": True,
    "--author": True, "--initials": True, "--date": True, "--out": True,
}
...
def parse_args(argv: list[str]) -> tuple[list[str], dict[str, str | bool]]:
    ...
    A misspelt --out used to fall through unread, so `add --uot copy.docx` wrote
    the comment into the caller's own document instead, and said ok.
    """
    args: list[str] = []
...
def comment_text(comment) -> str:
    out = []
    for el in iter_elements(comment):
...
def is_done(entry) -> bool:
    """Whether a w15:commentEx says its thread is resolved.

    Word writes the flag as an on/off value, so a thread another tool marked
    with "true" is as resolved as one marked "1".
    """
    return (entry.get(qn("w15:done")) or "0").lower() in ("1", "true", "on")


def parent_map(ex_root) -> dict[str, str]:
    out: dict[str, str] = {}
    for entry in ex_root.findall(qn("w15:commentEx")):
...
def thread_root(parents: dict[str, str], para_id: str) -> str:
    """The paraId that owns the thread a comment belongs to.

    Word's threads are one level deep - paraIdParent names the comment that
    opened the thread, never an intermediate reply - so this walks up instead
    of reading one hop, for documents another tool nested deeper. The seen set
    stops a malformed cycle from spinning.
    """
    current = para_id.upper()
    seen = {current}
...
def thread_members(ex_root, parents: dict[str, str], root_para: str) -> list:
    return [
        entry
        for entry in ex_root.findall(qn("w15:commentEx"))
        if thread_root(parents, (entry.get(qn("w15:paraId")) or "").upper()) == root_para.upper()
    ]
...
def story_anchors(pkg: Package) -> tuple[dict[str, str], dict[str, tuple[str, int]]]:
    """Anchored text and (part, paragraph) per comment id, across every story.

    A paragraph index only means anything beside the part that counts it, and a
    comment can be anchored in a header, footer or note as well as the body.
    """
    texts: dict[str, str] = {}
    located: dict[str, tuple[str, int]] = {}
    for name in sorted(pkg.stories()):
        story_texts, paras = anchors(pkg.tree(name))
        for cid, text in story_texts.items():
            texts.setdefault(cid, text)
        for cid, index in paras.items():
            located.setdefault(cid, (name, index))
    return texts, located


def anchor_story(pkg: Package, cid: str):
    """The story owning a comment's anchor, with its range start and reference.

    A reply has to be threaded into the story that holds the comment it
    answers, which is word/document.xml for most comments and a header, footer
    or note for the rest.
    """
    for name in sorted(pkg.stories()):
        start = ref = None
        for el in iter_elements(pkg.tree(name)):
            tag = local(el)
            if tag == "commentRangeStart" and el.get(qn("w:id")) == cid:
                start = el
            elif tag == "commentReference" and el.get(qn("w:id")) == cid:
                ref = el
        if ref is not None:
            return name, start, ref
...
def cmd_list(pkg: Package) -> dict:
    ...
    anchored, located = story_anchors(pkg)
    ...
    for comment in comment_elements(croot):
        ...
        part, at = located.get(cid, (None, None))
        records.append({
            "id": cid,
...
def cmd_add(pkg: Package, flags: dict) -> dict:
    n, text = flags.get("--paragraph"), flags.get("--text")
    ...
    comment, para_id = build_comment(pkg, cid, text, author,
                                     flags.get("--initials", initials_for(author)),
                                     flags.get("--date", now_stamp()), taken)
...
def cmd_reply(pkg: Package, flags: dict) -> dict:
    target, text = flags.get("--to"), flags.get("--text")
    ...
    story, parent_start, parent_ref = anchor_story(pkg, str(target))
    if parent_ref is None:
        fail(f"comment {target} has no reference in the document")
    pkg.touch(story)
    exroot = extended_root(pkg, create=True)
    taken = used_para_ids(croot, exroot, pkg.tree(IDS) if pkg.has(IDS) else None,
                          *(pkg.tree(name) for name in pkg.stories()))
    parent_para = ensure_para_id(parent, taken)
    ex_entry(exroot, parent_para)
    # Word's paraIdParent always names the thread root, so a reply aimed at
    # another reply joins that reply's thread; a deeper parent would make a
    # comment Word neither shows in the thread nor resolves with it.
    root_para = thread_root(parent_map(exroot), parent_para)
    ex_entry(exroot, root_para)
    cid = next_id(croot)
    author = flags.get("--author", "LangAlpha")
    comment, para_id = build_comment(pkg, cid, text, author,
                                     flags.get("--initials", initials_for(author)),
    ...
    entry.set(qn("w15:paraIdParent"), root_para)
    # A reply is a new turn in the conversation, so a thread someone had
    # resolved is open again, which is what Word shows once a reply lands in
    # one. Leaving the root marked done gives the file a resolved thread with
    # an unanswered reply inside it, and `list` reads the root alone, so the
    # reply would not be counted as an open thread anywhere.
    members = thread_members(exroot, parent_map(exroot), root_para)
    reopened = any(is_done(ex) for ex in members)
    for ex in members:
    ...
    return {"action": "reply", "id": cid, "to": str(target), "para_id": para_id,
            "parent_para_id": root_para, "reopened": reopened}


def cmd_resolve(pkg: Package, args: list[str], flags: dict, done: bool) -> dict:
    target = flags.get("--id") or (args[2] if len(args) > 2 else None)
    if target is None:
    ...
    ex_entry(exroot, para_id)
    parents = parent_map(exroot)
    root_para = thread_root(parents, para_id)
    ex_entry(exroot, root_para)
    touched = []
    for ex in thread_members(exroot, parents, root_para):
...
def cmd_strip(pkg: Package) -> dict:
    ...
    for name in pkg.stories():
        if any(local(el) in ANCHOR_TAGS for el in iter_elements(pkg.tree(name))):
            strip_anchors(pkg.touch(name))
    return {"action": "strip", "removed": removed,
            "parts_removed": [name for name in STRIP_PARTS if pkg.remove(name)]}
...
def main(argv: list[str]) -> None:
    ...
    if action == "list":
        ...
        return
    if action == "add":
        result = cmd_add(pkg, flags)
    elif action == "reply":
        result = cmd_reply(pkg, flags)
    elif action in ("resolve", "reopen"):
        result = cmd_resolve(pkg, args, flags, action == "resolve")
    elif action == "strip":
