...
and become #REF! when deleted entirely. A structured reference names a table
column instead of a grid position, so an insertion leaves it alone and deleting
the column it names turns it into #REF!, narrowing Table1[[Revenue]:[Cost]] to
the columns that survive, as Excel does; an edit that deletes a whole table
turns every reference naming it into #REF!, the bare Table1 and the [#All],
[#Data], [#Headers] and [#Totals] forms included. A 3-D reference such as
SUM(Jan:Mar!A1) addresses one coordinate on every sheet of a span, so an edit on
one sheet inside that span leaves it exactly as written, again as Excel does,
and a three_d_reference warning names each formula whose component on the
edited sheet now reads a different cell. External workbook links and pivot
...
Output is one JSON object on stdout:
    ...
    warnings            located #REF! occurrences, external links, 3-D spans,
                        dropped features, placeholder headers and other review
                        items
...
STRUCTURED = re.compile(r"([^\[\]!]+)\[(.*)\]\Z", re.S)
X14 = "http://schemas.microsoft.com/office/spreadsheetml/2009/9/main"
X15 = "http://schemas.microsoft.com/office/spreadsheetml/2010/11/main"
XM = "http://schemas.microsoft.com/office/excel/2006/main"
CHARTEX = "http://schemas.microsoft.com/office/drawing/2014/chart"
# Only Excel's own parts carry Excel formulas: a <formula> or <f> in any other namespace
# is a foreign payload riding in customXml or a third-party extLst, so rewriting its text
# corrupts somebody else's data and reading it as a formula fails the run over prose.
FORMULA_NS = {SML, CHART, X14, X15, XM, CHARTEX}
# The item specifiers a structured reference can carry. Anything else between its
# brackets is a column name, including a name that opens with an escaped '#.
SPECIFIERS = {"#all", "#data", "#headers", "#totals", "#this row"}
FORMULA_TAGS = {
    "formula",
    "formula1",
    "formula2",
...
def namespace(node):
    return etree.QName(node).namespace if isinstance(node.tag, str) else None
...
def structured_parts(inner):
    """Split a structured reference body into its #specifiers and its column names.

    Excel writes one reference as Table1[Revenue] or as Table1[[#Headers],[Revenue]],
    so both reduce to the same list here, with the ' escapes resolved and the "," and
    ":" that joined the items returned alongside them.
    """
    this_row = inner.startswith("@")
    if this_row:
        inner = inner[1:]
    items, separators, buffer, depth, i = [], [], [], 0, 0
    while i < len(inner):
        char = inner[i]
        ...
        i += 1
    items.append("".join(buffer).strip())
    return this_row, items, separators


def structured_text(table, this_row, specifiers, columns):
    body = ":".join(f"[{escape_column(c)}]" for c in columns)
    if specifiers:
        body = ",".join(f"[{s}]" for s in specifiers) + "," + body
    elif len(columns) == 1:
        body = escape_column(columns[0])
    return f"{table}[{'@' if this_row else ''}{body}]"
...
class Shift:
    def __init__(self, sheet, sheets, axis, at, count, delete):
        ...
        # Table name -> (column names in order, the ones this edit deletes), and the
        # casefolded names of the tables the edit removes outright. Filled in by the
        # Editor, which is the side that can read the table parts.
        self.tables = {}
    ...
    def sheet_names(self, qualifier):
        """The sheet names a qualifier spells, casefolded, or None when it names a book."""
        if "[" in qualifier:
            return None
        ...
        if len(names) == 1:
            names = names[0].split(":")
        return [s.casefold() for s in names]

    def three_d(self, qualifier):
        """Whether a qualifier spans sheets and runs across the one being edited.

        Excel leaves such a reference exactly as written when rows or columns move on
        one sheet inside the span: the single coordinate has to keep addressing every
        sheet in it, so shifting it would move the other sheets' components too, which
        is the one thing the edit did not do. Only the edited sheet's component ends up
        reading a different cell, which the report names.
        """
        names = self.sheet_names(qualifier)
        if names is None or len(names) != 2 or any(s not in self.sheets for s in names):
            return False
        first, last = sorted(self.sheets.index(s) for s in names)
        return first <= self.sheets.index(self.sheet.casefold()) <= last

    def moved(self, ref):
        """Whether this edit changes a reference, whatever sheet it is read against."""
        try:
            return self.range(ref) != ref
        except ValueError:
            return False
    ...
    def in_scope(self, qualifier, context):
        ...
        names = self.sheet_names(qualifier)
        # Two names are a 3-D span, which one sheet's edit never moves; three_d()
        # reports the spans this edit leaves reading a different cell instead.
        return names is not None and names == [self.sheet.casefold()]

    def structured(self, value):
        """Repoint Table1[Revenue] when this edit deletes the Revenue column.

        A structured reference names its column rather than a grid position, so the A1
        path below cannot see it and the sheet the formula sits on does not matter:
        Excel turns a reference at a deleted column into #REF! and narrows a column
        range to the columns that survive. An edit that takes the whole table with it
        leaves nothing for any of its references to name, so each one becomes #REF!
        whether or not it carries a column: the bare table name, which arrives here as
        a RANGE operand of its own, and the [#All], [#Data], [#Headers] and [#Totals]
        forms included.
        """
        match = STRUCTURED.match(value) if value.endswith("]") else None
        name = (match.group(1) if match else value).strip()
        if name.casefold() in self.dropped_tables:
            return "#REF!"
        if match is None:
            return value
        ...
        if table is None:
            return value
        order, dropped = table
        this_row, items, separators = structured_parts(match.group(2))
        specifiers = [item for item in items if item.casefold() in SPECIFIERS]
        named = [n for n, item in enumerate(items) if item.casefold() not in SPECIFIERS]
        if len(named) == 1:
            span = [items[named[0]]]
        elif (
            len(named) == 2 and named[1] == named[0] + 1 and separators[named[0]] == ":"
        ):
            span = [items[n] for n in named]
        else:
            return value
        places = {name.casefold(): n for n, name in enumerate(order)}
        bounds = [places.get(name.casefold()) for name in span]
        if None in bounds:
            return value  # A column this table never had; leave the formula as written.
        kept = [
            name
            for name in order[min(bounds) : max(bounds) + 1]
            if name.casefold() not in dropped
        ]
        if not kept:
            return "#REF!"
        ends = [kept[0]] if len(kept) == 1 else [kept[0], kept[-1]]
        if ends == span:
            return value
        return structured_text(match.group(1), this_row, specifiers, ends)

    def operand(self, value, context):
        if "#REF!" in value:
            return value
        if self.tables or self.dropped_tables:
            moved = self.structured(value)
            if moved != value:
                return moved
...
class Editor:
    def __init__(self, package, sheet, axis, at, count, delete, copy_from, report):
        ...
        self.shift.tables, self.shift.dropped_tables = self.deleted_table_parts()

    def deleted_table_parts(self):
        """What this edit takes from each table: its dropped columns, or the whole table.

        Read before anything is rewritten, because tables() removes the column
        definitions and the table parts themselves, so a formula naming one has nothing
        left to resolve against by the time its turn comes. A table is gone on exactly
        the condition tables() drops it on, so the two stay in step.
        """
        tables, gone = {}, set()
        if not self.shift.delete:
            return tables, gone
        for relation, part in self.p.relationships(self.part):
            ...
            if not ref:
                continue
            name = (table.get("displayName") or table.get("name") or "").casefold()
            if self.shift.range(ref) is None:
                gone.add(name)
                continue
            columns = table.find(tag("tableColumns"))
            if columns is None or self.shift.axis != "column":
                continue
            left = xy(ref.split(":")[0])[0]
            order = [c.get("name", "") for c in columns]
            dropped = {
                column.casefold()
    ...
    def other_formulas(self):
        ...
        for part in self.p.original:
            ...
            for node in root.iter():
                ...
                elif namespace(node) in FORMULA_NS and (
                    kind in FORMULA_TAGS or (kind == "f" and node.tag != tag("f"))
                ):
...
def formula_records(package):
    ...
    for part, data in package.original.items():
        ...
        for node in root.iter():
            kind = local(node)
            if namespace(node) in FORMULA_NS and (
                kind in FORMULA_TAGS | {"definedName"}
                or (kind == "f" and node.tag != tag("f"))
            ):
                if node.text:
...
def reference_warnings(package, shift):
    warnings = []
    for location, text in formula_records(package):
        ...
        for token in formula_tokens(text):
            if (
                ...
                qualifier, ref = token.value.rsplit("!", 1)
                if "[" in qualifier:
                    warnings.append(
                        {
                ...
                elif shift.three_d(qualifier) and shift.moved(ref):
                    spans.setdefault(qualifier, token.value)
        for span, reference in spans.items():
            warnings.append(
                {
                    "type": "three_d_reference",
                    **location,
                    "reference": reference,
                    "span": span,
                    "formula": text,
                    "message": "3-D reference left as written; its component on the edited sheet now reads a different cell",
                }
            )
    return warnings
...
def execute(argv):
    ...
    if flags.get("--dry-run"):
        report["dry_run"] = True
    editor = Editor(package, sheet, axis, at, count, delete, copy_from, report)
    editor.run()
    ...
    report["warnings"].extend(reference_warnings(computed, editor.shift))
    if flags.get("--dry-run"):
        buffer.seek(0)
