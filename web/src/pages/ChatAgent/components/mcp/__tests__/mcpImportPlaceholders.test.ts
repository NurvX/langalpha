...
describe('findPlaceholders on argv values a secret flag names', () => {
  ...
  it('leaves a secret-flag value carrying two different spans to the edit form', () => {
    // One answer cannot be both the user and the password, in either argv shape.
    expect(found(['--token', 'Basic <your-user>:<your-pass>'])).toEqual([]);
    expect(found(['--token=Basic <your-user>:<your-pass>'])).toEqual([]);
    // A single span under the same flag is still one answer, so it still asks.
    expect(found(['--token', 'Bearer <your-api-key>']).map((p) => p.literal)).toEqual([
      'Bearer <your-api-key>',
    ]);
  });

  it('imports the two-span values verbatim while a one-span value beside them fills', () => {
    const payload = blobOf([
      '--token',
      'Basic <your-user>:<your-pass>',
      '--secret=Basic <your-id>:<your-secret>',
    ...
    expect(out.mcpServers.tool.args).toEqual([
      '--token',
      'Basic <your-user>:<your-pass>',
      '--secret=Basic <your-id>:<your-secret>',
      '--api-key=sk-live',
...
describe('substitutePlaceholders fills the bracketed span, not the whole value', () => {
  ...
  it('fills every occurrence when a literal repeats one span', () => {
    const payload = {
      mcpServers: {
        acme: {
          type: 'http',
          url: 'https://acme.example.com/mcp',
          headers: { 'X-Key': 'id=<your-key> key=<your-key>' },
        },
      },
    };
    // One spelling is one answer, so the literal is still asked about.
    expect(
      findPlaceholders(parseMcpServersJson(JSON.stringify(payload)).servers).map((p) => p.literal),
    ).toEqual(['id=<your-key> key=<your-key>']);
    expect(
      fill(payload, 'id=<your-key> key=<your-key>', 'EXAMPLE-TOKEN').mcpServers.acme.headers[
        'X-Key'
      ],
    ).toBe('id=EXAMPLE-TOKEN key=EXAMPLE-TOKEN');
  });

  it('leaves a header carrying two different spans alone, since one answer would fill both', () => {
    const payload = {
      mcpServers: {
        acme: {
          type: 'http',
    ...
    // Collecting it asked once, under a password label, and wrote that answer
    // into the username too. Only the field one answer can fill is asked for.
    expect(
      findPlaceholders(parseMcpServersJson(JSON.stringify(payload)).servers).map((p) => p.literal),
    ).toEqual(['<your-api-key>']);
    const out = fill(payload, '<your-api-key>', 'EXAMPLE-TOKEN');
    expect(out.mcpServers.acme.headers.Authorization).toBe('Basic <your-username>:<your-password>');
    expect(out.mcpServers.acme.headers['X-Key']).toBe('EXAMPLE-TOKEN');
  });

  it('leaves a DSN naming both a user and a password to the edit form', () => {
    const payload = {
      mcpServers: {
        acme: {
          command: 'npx',
          args: ['-y', 'thing'],
          env: {
            DATABASE_URL: 'postgresql://<your-user>:<your-password>@localhost:5432/db',
            TOKEN: '<your-api-key>',
          },
        },
      },
    ...
    const out = fill(payload, '<your-api-key>', 'EXAMPLE-TOKEN');
    expect(out.mcpServers.acme.env.DATABASE_URL).toBe(
      'postgresql://<your-user>:<your-password>@localhost:5432/db',
    );
    expect(out.mcpServers.acme.env.TOKEN).toBe('EXAMPLE-TOKEN');
  });
});
