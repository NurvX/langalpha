...
// A literal carrying two different spans has no one answer that is right for
// both, so it imports exactly as written and the user fills it in the edit form.
function oneAnswerFills(value: string): boolean {
  return new Set(value.match(ANGLE_SEGMENT) ?? []).size < 2;
}
...
export function findPlaceholders(servers: ParsedImportServer[]): Placeholder[] {
  ...
  const note = (s: ParsedImportServer, literal: string, use: PlaceholderUse) => {
    if (!oneAnswerFills(literal)) return;
