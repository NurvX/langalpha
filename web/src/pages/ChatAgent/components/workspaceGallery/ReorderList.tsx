...
import { getAllReorderWorkspaces } from './loadReorderWorkspaces';
...
export function ReorderList({ flashWorkspace, onDone }: ReorderListProps) {
  ...
  // Reorder needs every row before it can write a complete ordering. Keep
  // paging until the server's total is satisfied instead of silently omitting
  // everything after the first API page.
  const {
    data: allWorkspaces,
    isLoading,
    isError,
  } = useQuery({
    queryKey: queryKeys.workspaces.list({ view: 'reorder-all', sortBy: 'custom' }),
    queryFn: getAllReorderWorkspaces,
    staleTime: 0,
  ...
  useEffect(() => {
    ...
    setRows(flashWorkspace ? [flashWorkspace, ...allWorkspaces] : allWorkspaces);
  }, [allWorkspaces, flashWorkspace]);
